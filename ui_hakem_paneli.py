...
def hakem_panelini_ciz():
    ...
    with sekme_arama:
        ...
        with col1:
            if st.button("ITF Kuralları", use_container_width=True): st.session_state.aktif_kategori = "ITF Kuralları"
            if st.button("Men's WTT", use_container_width=True): st.session_state.aktif_kategori = "Men's WTT"
            if st.button("Women's WTT", use_container_width=True): st.session_state.aktif_kategori = "Women's WTT"
            if st.button("WTT Juniors", use_container_width=True): st.session_state.aktif_kategori = "WTT Juniors"
                
        with col2:
            if st.button("WTT Masters", use_container_width=True): st.session_state.aktif_kategori = "WTT Masters"
            if st.button("Wheelchair Tour", use_container_width=True): st.session_state.aktif_kategori = "Wheelchair Tour"
            if st.button("Beach Tennis", use_container_width=True): st.session_state.aktif_kategori = "Beach Tennis"
            if st.button("Tennis Europe", use_container_width=True): st.session_state.aktif_kategori = "Tennis Europe"
                
        with col3:
            if st.button("ATP", use_container_width=True): st.session_state.aktif_kategori = "ATP"
            if st.button("WTA", use_container_width=True): st.session_state.aktif_kategori = "WTA"
            if st.button("Grand Slam", use_container_width=True): st.session_state.aktif_kategori = "Grand Slam"
                
        with col4:
            if st.button("TTF Ulusal", use_container_width=True): st.session_state.aktif_kategori = "TTF Ulusal"
            if st.button("Ulusal Diğer", use_container_width=True): st.session_state.aktif_kategori = "Ulusal Diğer"
            if st.button("Sık Sorulanlar", use_container_width=True): st.session_state.aktif_kategori = "Sık Sorulanlar"
        ...
        if st.session_state.aktif_kategori == "Kategori Seçilmedi":
            ...
            aranan_kelime = st.chat_input("Aranacak kelimeyi yazın (Örn: foot fault, mola)...")
            ...
            if aranan_kelime:
                ...
                with st.chat_message("assistant"):
                    with st.spinner("Aranıyor..."):
                        try:
                            # 1. KATI NORMALLAŞTIRMA (Sadece birleşik, ayrı ve tireli ihtimaller)
                            aranan_ilk = aranan_kelime.lower().strip()
                            aranan_ilk = re.sub(r'\s+', ' ', aranan_ilk)
                            
                            temel_terimler = {aranan_ilk}
                            
                            # KATI SÖZLÜK ARAMASI: İçinde geçiyorsa değil, TAM eşleşiyorsa İngilizcelerini getir!
                            ...
                            if sonuclar:
                                ...
                                st.warning(f"'{aranan_kelime}' seçilen kategoride bulunamadı.")
                        except Exception as e:
                            st.error(f"Arama sırasında hata oluştu: {e}")

    with sekme_indeks:
        st.subheader("Kayıtlı Belgeler Kütüphanesi")
        # İndeks kısmı aynı kalıyor...
        try:
            ...
            if response.data:
                ...
                siralama_turu = st.radio("Filtreleme Modu:", ["Alfabetik Sıralama (Tümü)", "Kategoriye Göre Sıralama"], horizontal=True)
