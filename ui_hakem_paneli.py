...
def yapay_zeka_ayarla():
    try:
        ...
        # En yetenekli modeli seçiyoruz
        model = genai.GenerativeModel('gemini-1.5-pro')
        return model
    except Exception as e:
        st.error("Gemini API anahtarı Secrets'ta bulunamadı.")
        return None
...
def hakem_panelini_ciz():
    ...
    with sekme_arama:
        ...
        with col1:
            if st.button("ITF Kuralları", key="btn1"): st.session_state.aktif_kategori = "ITF Kuralları"
            if st.button("Men's WTT", key="btn2"): st.session_state.aktif_kategori = "Men's WTT"
            if st.button("Women's WTT", key="btn3"): st.session_state.aktif_kategori = "Women's WTT"
            if st.button("WTT Juniors", key="btn4"): st.session_state.aktif_kategori = "WTT Juniors"
                
        with col2:
            if st.button("WTT Masters", key="btn5"): st.session_state.aktif_kategori = "WTT Masters"
            if st.button("Wheelchair Tour", key="btn6"): st.session_state.aktif_kategori = "Wheelchair Tour"
            if st.button("Beach Tennis", key="btn7"): st.session_state.aktif_kategori = "Beach Tennis"
            if st.button("Tennis Europe", key="btn8"): st.session_state.aktif_kategori = "Tennis Europe"
                
        with col3:
            if st.button("ATP", key="btn9"): st.session_state.aktif_kategori = "ATP"
            if st.button("WTA", key="btn10"): st.session_state.aktif_kategori = "WTA"
            if st.button("Grand Slam", key="btn11"): st.session_state.aktif_kategori = "Grand Slam"
                
        with col4:
            if st.button("TTF Ulusal", key="btn12"): st.session_state.aktif_kategori = "TTF Ulusal"
            if st.button("Ulusal Diğer", key="btn13"): st.session_state.aktif_kategori = "Ulusal Diğer"
            if st.button("Sık Sorulanlar", key="btn14"): st.session_state.aktif_kategori = "Sık Sorulanlar"
                ...
                with st.chat_message("assistant"):
                    with st.spinner("Sözlük taranıyor..."):
                        try:
                            ...
                            sorgu = supabase.table("kural_icerikleri").select("dosya_adi, kategori, sayfa_no, dosya_url, icerik").eq("kategori", st.session_state.aktif_kategori)
                            ...
                            if sonuclar:
                                ...
                                for kayit in sonuclar:
                                    ...
                                    if idx != -1:
                                        ...
                                        vurgulu_kesit = pattern.sub(lambda m: f'<span style="background-color: #39ff14; color: #000000; font-weight: bold;">{m.group(0)}</span>', kesit)
    ...
    with sekme_ai:
        ...
        if st.session_state.aktif_kategori == "Kategori Seçilmedi":
            st.warning("Lütfen yukarıdaki Arama sekmesinden geçerli bir Kategori seçin.")
        elif not ai_model:
            st.warning("Gemini API henüz ayarlanmamış.")
        else:
            st.info(f"Yapay Zeka şu an **{st.session_state.aktif_kategori}** kurallarını baz alarak cevap verecek.")
            
            # Telefonda bu alana tıklandığında klavyede direkt "Mikrofon/Dikte" tuşuna basarak konuşabilirsiniz.
            ...
            if olay_metni:
                ...
                with st.chat_message("assistant"):
                    with st.spinner(f"Gemini {st.session_state.aktif_kategori} metinlerini okuyor ve olayı analiz ediyor..."):
                        try:
                            # 1. Supabase'den seçili kategorideki tüm metinleri çek
                            ...
                            if not tum_metin:
                                st.warning("Bu kategoride henüz kural metni bulunmuyor. Lütfen Admin panelinden PDF yükleyin.")
                            else:
                                # 2. Gemini'a talimatı (Prompt) gönder
                                prompt = f"""
            ...
            if response.data:
                ...
                if siralama_turu == "Alfabetik Sıralama":
                    ...
                    for idx, row in df_unique.iterrows():
                        st.markdown(f"**{row['dosya_adi']}** - {row['kategori']}")
                else:
                    kategoriler_listesi = df_unique["kategori"].unique().tolist()
                    if kategoriler_listesi:
                        secilen_grup = st.selectbox("Kategori Seçin:", kategoriler_listesi)
