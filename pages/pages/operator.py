        ...
        if st.button("Belgeleri Yükle ve Sayfa Sayfa İşle", type="primary"):
            if yuklenen_dosyalar:
                ...
                for dosya in yuklenen_dosyalar:
                    ...
                    try:
                        # 1. Veritabanındaki eski kayıtları temizle (Duplicate önlemi)
                        supabase.table("kural_icerikleri").delete().eq("dosya_adi", temiz_dosya_adi).execute()
                        
                        # 2. Depodaki eski dosyayı silmeyi dene
                        try:
                            supabase.storage.from_("Belgeler").remove([temiz_dosya_adi])
                        except:
                            pass 
                        
                        # 3. Tertemiz yeni dosyayı yükle
